		...
		if (searchResults) {
			for(var i = 0, y = searchResults.length; i < y; i++) {
				...
				if(oldValue !== newValue) {
					var currentRec = nlapiLoadRecord(recordType, recordId);
					currentRec.setFieldValue(fieldId, newValue);

					// set the date too
					currentRec.setFieldValue('custentity_pending_status_date', DateNow());
